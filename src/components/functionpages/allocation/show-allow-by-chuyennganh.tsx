...
import { phanbosinhvientheochuyennganh, phanBoTuDongchuyennganh } from "../../../functions/internship-allocation";
import type { CompanychuyennganhPayload , StudentchuyennganhPayload } from "../../../models/allocation/add"

interface AllocationCompanyWithStudents extends CompanychuyennganhPayload {
  students: (StudentchuyennganhPayload & { distance: number })[];
}
...
export default function XacthucPhanBoTuDongChuyenNganhCard({
  onClose,
  students,
  companies,
  madot,
  ...
  useEffect(() => {
    setLoading(true);
    phanbosinhvientheochuyennganh(students, companies)
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      alert(JSON.stringify({ allocations }, null, 2));
      await phanBoTuDongchuyennganh(madot, madotphanbo, allocations);
          ...
          >
            ...
            <div className="bg-blue-50 px-6 py-4 border-b border-border flex justify-between items-start">
              <div className="flex-1">
                ...
                <p className="text-sm text-slate-600 break-words whitespace-pre-wrap">{company.chuyennganh}</p>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 max-h-80 overflow-y-auto">
                  {company.students.map((student, idx) => (
                    ...
                    >
                      ...
                      <span className="text-sm text-slate-600 break-words whitespace-pre-wrap">{student.chuyennganh}</span>
