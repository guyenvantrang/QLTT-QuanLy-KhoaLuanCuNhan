...
import { 
    FaSearch, FaTimes, FaFileExcel, FaEye, 
    FaUserGraduate, FaBuilding, FaChalkboardTeacher, FaClock, FaInfoCircle
} from "react-icons/fa";
import type { SinhVien } from "../../../models/model-all";
import { layTongSinhVienCoCongTy, laysinhvientheoid } from "../../../functions/student";

// Dùng thư viện hỗ trợ Style cho Excel
...
const getStudentResult = (sv: SinhVien, madot: string) => {
    ...
    if (!diem || diem.muctieuthuctap === null || diem.muctieuthuctap === undefined) {
        return { 
            status: "pending", total: 0, label: "Chưa chấm",
            components: { kienthuc: 0, kynang: 0, thaido: 0 },
            raw: null
        };
    }

    const d_thaido = diem.ythuckyluat || 0;
    const d_kienthuc = diem.kienthucthuctien || 0;
    ...
    return {
        ...
        components: { thaido: round(d_thaido), kienthuc: round(d_kienthuc), kynang: round(d_kynang) },
        raw: diem
    };
};

// ==========================================
// 2. SUB-COMPONENTS (Badge, Box, Modal)
// ==========================================

const ResultBadge = ({ status, score }: { status: string, score: number }) => {
    if (status === "pending") {
        return <span className="px-3 py-1 rounded-full text-xs font-bold bg-slate-100 text-slate-500 border border-slate-200 flex items-center gap-1 w-fit"><FaInfoCircle /> Chưa chấm</span>;
    }
    const colorClass = status === "pass" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-200";
    return (
        <div className="flex flex-col items-start">
            <span className={`px-3 py-1 rounded-full text-xs font-bold border mb-1 ${colorClass}`}>{status === "pass" ? "Đạt yêu cầu" : "Không đạt"}</span>
...
const ScoreBox = ({ label, score, max, color, isBig = false }: { label: string, score: any, max: number, color: string, isBig?: boolean }) => {
    ...
    return (
        <div className={`flex flex-col justify-between p-3 rounded-xl border ${colors[color]} ${isBig ? "md:col-span-2 lg:col-span-1 ring-1 ring-emerald-200" : ""}`}>
            ...
            <div className={`font-black ${isBig ? "text-3xl" : "text-2xl"} text-right tracking-tighter`}>{score}</div>
        </div>
    );
};
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[95vh] overflow-hidden flex flex-col border border-white/20">
                <div className="p-5 border-b border-slate-100 bg-gradient-to-r from-blue-700 to-blue-600 text-white flex justify-between items-start">
                    ...
                    <button onClick={onClose} className="p-2 hover:bg-white/20 rounded-full transition-colors text-white/80 hover:text-white"><FaTimes size={24} /></button>
                </div>
                ...
                <div className="p-6 overflow-y-auto bg-slate-50/50 flex-1 custom-scrollbar">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div className="md:col-span-2 bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                            <h4 className="text-sm font-bold text-slate-700 uppercase mb-4 flex items-center gap-2 border-b border-slate-100 pb-2"><FaUserGraduate className="text-blue-600" /> Thông tin thực tập</h4>
...
export default function StudentHaveCompanyManagement() {
    ...
    // --- Load Data (Chỉ lấy sinh viên có công ty) ---
    useEffect(() => {
        if (!madot) return;
        setLoading(true);
        // GỌI API LẤY SINH VIÊN CÓ CÔNG TY
        layTongSinhVienCoCongTy(madot)
            .then(async (res) => {
    ...
    const exportExcel = () => {
        ...
        const row2 = [{ v: "TRƯỜNG ĐẠI HỌC CÔNG NGHỆ...", s: styleTitle }, null, null, null, null, { v: "Độc lập - Tự do - Hạnh phúc", s: { ...styleTitle, font: { ...styleTitle.font, underline: true } } }];
        const rowTitle = [{ v: "DANH SÁCH SINH VIÊN ĐÃ CÓ CÔNG TY & BẢNG ĐIỂM", s: { ...styleTitle, font: { ...styleTitle.font, sz: 16 } } }];
        const rowBatch = [{ v: `Đợt: ${madot}`, s: { ...styleTitle, font: { ...styleTitle.font, italic: true } } }];

        // Table Headers
        const headers = ["STT", "Mã SV", "Họ và Tên", "Ngày sinh", "Lớp", "Đơn vị thực tập", 
                         "Mục tiêu\n(0.75)", "GT Đơn vị\n(0.75)", "Vị trí\n(0.5)", 
                         "Vận dụng\n(0.5)", "Phương pháp\n(0.5)", 
                         "Yêu cầu\n(0.5)", "Báo cáo\n(0.5)", "Thuyết trình\n(0.5)", "Hỏi đáp\n(0.5)", 
                         "Ý thức\n(2.0)", "Thực tiễn\n(3.0)", 
        ...
        XLSX.utils.book_append_sheet(workbook, ws, "DS_DaCoCongTy");
        XLSX.writeFile(workbook, `DS_DaCoCongTy_${madot}_${new Date().toLocaleDateString('vi-VN').replaceAll('/','-')}.xlsx`);
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                <div>
                    <h2 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-700 to-indigo-600 flex items-center gap-2">
                        <FaBuilding className="text-blue-600" /> Sinh Viên Đã Có Công Ty
                    </h2>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-blue-50/50 border-b border-blue-100 text-left">
                                    ...
                                    <th className="px-6 py-4 text-xs font-bold text-blue-800 uppercase">Công ty & GVHD</th>
                                        ...
                                        return (
                                            <tr key={sv.masv} className="hover:bg-blue-50/30 transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-sm">
                                                    <p className="font-semibold text-slate-700 flex items-center gap-1.5"><FaBuilding className="text-blue-400" /> {dk?.macongty || "---"}</p>
                                                    <p className="text-xs text-slate-500 flex items-center gap-1.5 mt-1"><FaChalkboardTeacher className="text-purple-400" /> GV: {dk?.magiangvien || "---"}</p>
                                                </td>
