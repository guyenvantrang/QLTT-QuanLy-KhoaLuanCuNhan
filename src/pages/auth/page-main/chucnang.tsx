...
import { getAllChucNang, deleteChucNang } from "../../../api/login"; 

// Import Modal (Giả định bạn sẽ cập nhật các file này sau)
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm transition-opacity">
            <div className={`bg-white rounded-2xl shadow-2xl w-full ${sizeClass} max-h-[90vh] flex flex-col animate-in fade-in zoom-in duration-200`}>
                <div className="flex justify-between items-center p-5 border-b border-gray-100 bg-gradient-to-r from-indigo-50 to-white rounded-t-2xl shrink-0">
...
export default function QuanLyChucNang() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setListChucNang(Array.isArray(data) ? data : []);
        } catch (err: any) {
    ...
    return (
        <div className="p-6 bg-gradient-to-br from-slate-50 to-indigo-50/50 min-h-screen font-sans">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-gray-50/80 border-b border-gray-100">
                            <tr className="text-xs uppercase tracking-wider text-gray-500 font-bold">
                                ...
                                <th className="px-6 py-4 w-[20%]">Trang Truy Cập (URL)</th>
                        ...
                        <tbody className="divide-y divide-gray-50 text-sm text-gray-700">
                            {loading ? (
                                ...
                                <tr key={item.machucnang} className="hover:bg-indigo-50/30 transition-colors group">
                                    ...
                                    {/* 2. Tên chức năng */}
                                    <td className="px-6 py-4 font-semibold text-gray-800 group-hover:text-indigo-700 transition-colors">
                                        {item.tenchucnang}
                                    </td>

                                    {/* 3. Mã truy cập (Code để check quyền) */}
                                    <td className="px-6 py-4">
                                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-slate-100 text-slate-600 font-mono text-xs border border-slate-200">
                                            <FaKey className="w-3 h-3 opacity-50"/> {item.matruycap || "N/A"}
                                        </span>
                                    </td>

                                    {/* 4. Trang truy cập (URL/Path) */}
                                    <td className="px-6 py-4">
                                        {item.trangtruycap ? (
                                            <div className="flex items-center gap-2 text-blue-600 bg-blue-50/50 px-2 py-1 rounded-lg w-fit" title={item.trangtruycap}>
                                                <FaGlobe className="w-3 h-3"/> 
                                                <span className="font-medium text-xs truncate max-w-[150px]">{item.trangtruycap}</span>
                                            </div>
                                        ) : (
                                            <div className="flex items-center gap-2 text-gray-400 text-xs italic">
                                                <FaLink className="w-3 h-3"/> Chưa gán
                                            </div>
                                        )}
                                    </td>

                                    {/* 5. Mô tả */}
                                    <td className="px-6 py-4 text-gray-500 truncate max-w-xs text-xs">
                                        {item.mota || '-'}
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <div className="flex justify-center gap-2 opacity-80 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button 
                                                onClick={() => handleDelete(item.machucnang, item.tenchucnang || '')} 
            ...
            <CustomModal 
                show={modalOpen} 
                title={isEditing ? "Cập nhật Chức Năng" : "Thêm Chức Năng Mới"} 
            ...
            {selectedItem && (
                <CustomModal 
                    ...
                    title={`Chi Tiết Chức Năng [${selectedItem.machucnang}]`} 
