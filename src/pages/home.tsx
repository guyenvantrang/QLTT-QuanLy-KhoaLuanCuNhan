...
export default function HomePage() {
  ...
  const features = [
    ...
    },
    {
      ...
      stats: [
        { label: "Thông báo mới", value: 12, color: "#3B82F6" },
        { label: "Tổng số thông báo", value: 230, color: "#2563EB" },
      ],
      gradient: "from-blue-100 via-blue-200 to-blue-100",
      accessCode: "hienthi_thongbao",
      ...
      <main className="flex-1 p-6 max-w-7xl mx-auto">
        <div className="flex flex-col mb-8 max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center border-b border-gray-300 pb-2">
            {/* Quản lý thực tập luôn hiển thị */}
            <button
              className="flex items-center gap-2 text-xl font-bold px-4 py-2 text-indigo-700 border-b-4 border-indigo-700 transition-colors"
              onClick={() => navigate('/home')}
            >
              <FaHandshake className="w-6 h-6" /> Quản lý thực tập
            </button>

            {/* Quản lý hệ thống: dùng Can để kiểm soát quyền */}
            <Can trangtruycap="/home" matruycap="hienthi_phanquyen">
              <button
                className="flex items-center gap-2 text-xl font-medium px-4 py-2 text-gray-500 hover:text-indigo-600 transition-colors"
                onClick={() => navigate('/phanquyen')}
              >
        ...
        {/* Grid các chức năng chính */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature) => (
            <Can key={feature.title} trangtruycap="/home" matruycap={feature.accessCode}>
              <div
